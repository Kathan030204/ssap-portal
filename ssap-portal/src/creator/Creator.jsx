...
export function Creator({ onLogout }) {
  ...
  useEffect(() => {
    ...
    if (savedUser) {
      ...
      setUser({ id: uid, name: savedUser.username || savedUser.name, role: savedUser.role, loggedIn: true });
      fetchSections(uid);
    } else {
      onLogout();
    }
  ...
  const filteredSections = sections.filter(sec => {
    ...
    if (statusFilter === 'In Review') return sec.current_status === 'Pending Allocation' || sec.current_status === 'In Testing';
  ...
  return (
    <div className="flex min-h-screen bg-slate-50 font-sans text-slate-800">
      ...
      <aside className={`
        fixed inset-y-0 left-0 z-50 w-64 bg-slate-900 text-slate-300 flex flex-col transition-transform duration-300 lg:translate-x-0 lg:sticky lg:top-0 lg:h-screen
        ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}
      `}>
        <div className="p-6 border-b border-slate-800 flex justify-between items-center">
          <h1 className="text-xl font-black text-white tracking-tighter italic">CREATOR HUB</h1>
          <button className="lg:hidden text-slate-400" onClick={() => setIsSidebarOpen(false)}><FaTimes /></button>
        </div>

        <nav className="flex-1 p-4 space-y-2 mt-4 overflow-y-auto">
          <button
            onClick={() => { setActiveTab('inventory'); setStatusFilter('All'); setSelectedIssue(null); setIsSidebarOpen(false); }}
          ...
          {['In Review', 'Fix Required', 'Published'].map((f) => (
            <button
              key={f}
              onClick={() => { setActiveTab('inventory'); setStatusFilter(f); setSelectedIssue(null); setIsSidebarOpen(false); }}
          ...
          <button
            onClick={() => { setActiveTab('submit'); setIsEditing(null); setSelectedIssue(null); setFormData({ title: '', category: 'Hero', docs: '', live_link: '', shopify_admin_link: '' }); setIsSidebarOpen(false); }}
      ...
      <main className="flex-1 p-4 md:p-10 overflow-x-hidden">
        <header className="mb-10 flex justify-between items-start">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <h3 className="text-2xl md:text-3xl font-black text-slate-900 italic">Work Console</h3>
            </div>
          </div>

          <div className="relative" ref={notifRef}>
            <button onClick={() => setShowNotifDropdown(!showNotifDropdown)} className="p-3 md:p-4 rounded-2xl bg-white border border-slate-200 text-slate-400 hover:text-indigo-600 transition-all relative cursor-pointer">
              <FaBell size={20} />
              {notifications.length > 0 && <span className="absolute top-2 right-2 md:top-3 md:right-3 w-4 h-4 bg-rose-500 border-2 border-white rounded-full flex items-center justify-center text-[8px] text-white font-black">{notifications.length}</span>}
            </button>
            {showNotifDropdown && (
              <div className="absolute right-0 top-full mt-4 w-72 md:w-80 bg-white rounded-3xl shadow-2xl border border-slate-100 p-6 z-50">
        ...
        {activeTab === 'inventory' ? (
          <div className={`grid ${statusFilter === 'Fix Required' ? 'grid-cols-1 lg:grid-cols-12' : 'grid-cols-1'} gap-8`}>
            {/* Table Column */}
            <div className={statusFilter === 'Fix Required' ? 'lg:col-span-8 order-2 lg:order-1' : 'col-span-1'}>
              ...
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                <div className="overflow-x-auto"> {/* Added horizontal scroll for mobile table */}
            ...
            {statusFilter === 'Fix Required' && (
              <div className="lg:col-span-4 order-1 lg:order-2">
                <div className="bg-white rounded-3xl border border-slate-200 p-6 md:p-8 lg:sticky lg:top-10 shadow-sm min-h-100">
                  {selectedIssue ? (
          ...
          <div className="max-w-2xl mx-auto py-4 md:py-10">
            <form onSubmit={handleSubmission} className="bg-white rounded-4xl md:rounded-4xl border border-slate-200 shadow-2xl overflow-hidden">
              <div className={`${isEditing ? 'bg-indigo-600' : 'bg-slate-800'} p-6 md:p-10 text-white text-center transition-colors`}>
              ...
              <div className="p-6 md:p-10 space-y-6 md:y-8">
                <div>
                  ...
                  <input
                    ...
                    className="w-full border-2 border-slate-50 bg-slate-50 rounded-2xl p-4 md:p-5 text-sm font-bold outline-none focus:border-indigo-500 disabled:opacity-70"
                ...
                <div>
                  ...
                  <div className="p-6 md:p-10 border-4 border-dashed border-indigo-50 bg-indigo-50/20 rounded-3xl text-center group hover:border-indigo-200 transition-all">
                    <FaUpload className="mx-auto text-indigo-200 mb-4" size={32} />
                    <p className="text-xs md:text-sm font-black italic mb-6 break-all">{file ? file.name : 'Drag & Drop code package'}</p>
                    <label className="bg-indigo-600 text-white px-8 py-3 rounded-xl text-xs font-black cursor-pointer shadow-lg hover:bg-indigo-700 inline-block">
                      BROWSE <input type="file" className="hidden" onChange={(e) => setFile(e.target.files[0])} />
                    </label>
                  </div>
                </div>
                <div className="flex flex-col-reverse md:flex-row justify-end gap-4 md:gap-6 pt-4">
                  <button type="button" onClick={() => setActiveTab('inventory')} className="py-2 text-xs font-black uppercase text-slate-400 cursor-pointer">Cancel</button>
